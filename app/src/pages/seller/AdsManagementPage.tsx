...
export function AdsManagementPage() {
    ...
    const [adBalance, setAdBalance] = useState(500000);
    ...
    const handleViewCampaign = (id: string) => {
        // Toggle expanded view or navigate to detail
    ...
    const filteredCampaigns = campaigns.filter(c => {
        if (activeTab === 'Berjalan') return c.status === 'active';
        if (activeTab === 'Dijadwalkan') return c.status === 'scheduled';
        if (activeTab === 'Selesai') return c.status === 'ended';
        return true;
    });
        ...
        <MobileContainer className="bg-background-light dark:bg-background-dark min-h-screen font-display text-slate-900 dark:text-white antialiased pb-24">
            ...
            <div className="sticky top-0 z-50 bg-background-light dark:bg-background-dark/95 backdrop-blur-md border-b border-white/5 dark:border-[#54473b]/30 px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3 max-w-6xl mx-auto w-full">
                    <button
                        ...
                        className="flex items-center justify-center w-10 h-10 rounded-full hover:bg-white/5 active:scale-95 transition-all text-text-secondary dark:text-[#baaa9c] hover:text-primary"
                    >
                        <Icon name="arrow_back" size={24} />
                    </button>
                    <h1 className="text-xl font-bold leading-tight tracking-tight flex-1">Iklan Penjual</h1>
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="flex items-center justify-center w-10 h-10 rounded-full bg-primary text-white shadow-lg shadow-primary/20 active:scale-95 transition-all"
                        >
            ...
            <div className="flex flex-col w-full max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-4">
                        ...
                        <div className="relative overflow-hidden rounded-xl border border-[#54473b] dark:border-[#54473b] bg-surface-light dark:bg-[#2f251e] p-5 shadow-sm">
                            ...
                            <div className="relative z-10 flex flex-row items-center justify-between gap-4">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <p className="text-text-main dark:text-white text-xl font-bold leading-tight">{formatPrice(adBalance)}</p>
                                </div>
                                <button
                                    onClick={() => setShowTopUpModal(true)}
                                    className="flex items-center justify-center rounded-lg h-9 px-5 bg-primary hover:bg-primary/90 text-white text-sm font-bold shadow-lg shadow-primary/20 transition-all active:scale-95"
                                >
                    ...
                    {/* Right Column - Chart & Campaigns */}
                    <div className="lg:col-span-2 space-y-4">
                        {/* Charts Section */}
                        <div className="bg-surface-light dark:bg-[#2f251e] border border-gray-200 dark:border-[#54473b] rounded-xl p-4">
                        ...
                        {/* Campaign Tabs */}
                        <div className="bg-surface-light dark:bg-[#2f251e] border border-gray-200 dark:border-[#54473b] rounded-xl overflow-hidden">
                            <div className="flex border-b border-gray-200 dark:border-[#54473b] px-4 gap-6">
                                {tabs.map((tab) => (
                                    <button
                                        key={tab}
                                        onClick={() => setActiveTab(tab)}
                                        className="relative pb-3 pt-3 flex flex-col items-center"
                                    >
                                        <span className={`text-sm font-medium ${activeTab === tab ? 'text-primary font-bold' : 'text-text-secondary dark:text-[#baaa9c] hover:text-primary'} transition-colors`}>
                                            {tab}
                                        </span>
                            ...
                            <div className="p-4 flex flex-col gap-4">
                                {filteredCampaigns.length === 0 ? (
                                    <div className="py-8 text-center">
                                        <Icon name="campaign" size={48} className="text-gray-300 dark:text-gray-600 mx-auto mb-2" />
                                        <p className="text-text-secondary dark:text-gray-400">Belum ada kampanye</p>
                                        <button
                                            onClick={handleCreateCampaign}
                                    ...
                                    filteredCampaigns.map((campaign) => (
                                        <div
                                            key={campaign.id}
                                            className={`bg-white dark:bg-[#3a2e26] border border-gray-200 dark:border-[#54473b] rounded-xl p-3 flex gap-3 items-center ${campaign.status === 'scheduled' ? 'opacity-80' : ''}`}
                                        >
                                            {/* Product Image */}
                                            <button
                                                onClick={() => handleViewCampaign(campaign.id)}
                                                className="w-16 h-16 shrink-0 rounded-lg overflow-hidden bg-white/5 relative hover:opacity-80 transition-opacity"
                                            >
                                                <img
                                                    src={campaign.image}
                                                    alt={campaign.name}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start mb-1">
                                                    <button
                                                        onClick={() => handleViewCampaign(campaign.id)}
                                                        className="text-text-main dark:text-white font-bold text-sm truncate pr-2 hover:text-primary transition-colors text-left"
                                                    >
                                                        {campaign.name}
                                                    </button>
                                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border shrink-0 ${campaign.status === 'active'
            ...
            {showTopUpModal && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-surface-light dark:bg-surface-dark rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                        <h3 className="text-lg font-bold text-text-main dark:text-white mb-4">Top Up Saldo Iklan</h3>
                        <div className="mb-4">
                            <label className="text-sm text-text-secondary dark:text-gray-400 mb-2 block">Jumlah Top Up</label>
                            <input
                                type="text"
                                value={topUpAmount}
                                onChange={(e) => {
                                    const value = e.target.value.replace(/\D/g, '');
                                    setTopUpAmount(value ? formatPrice(parseInt(value)) : '');
                                }}
                        ...
                        <div className="flex gap-2 mb-4">
                            {[50000, 100000, 200000, 500000].map(amount => (
                                <button
                                    key={amount}
                                    onClick={() => setTopUpAmount(formatPrice(amount))}
                                    className="flex-1 py-2 px-2 text-xs font-medium rounded-lg border border-gray-200 dark:border-white/10 hover:border-primary hover:text-primary transition-colors"
                                >
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-background-light dark:bg-background-dark/95 backdrop-blur-lg border-t border-gray-200 dark:border-[#54473b]">
                <div className="flex justify-around items-center h-16 max-w-6xl mx-auto">
                    <button onClick={() => navigate('/seller-center')} className="flex flex-col items-center justify-center gap-1 w-full h-full text-text-secondary dark:text-[#baaa9c] hover:text-primary transition-colors">
                        <Icon name="home" size={24} />
                        <span className="text-[10px] font-medium">Home</span>
                    </button>
                    <button onClick={() => navigate('/seller/products')} className="flex flex-col items-center justify-center gap-1 w-full h-full text-text-secondary dark:text-[#baaa9c] hover:text-primary transition-colors">
                    ...
                    <button onClick={() => navigate('/seller/orders')} className="flex flex-col items-center justify-center gap-1 w-full h-full text-text-secondary dark:text-[#baaa9c] hover:text-primary transition-colors">
                        <Icon name="receipt_long" size={24} />
                        <span className="text-[10px] font-medium">Pesanan</span>
                    </button>
                    <button onClick={() => navigate('/profile')} className="flex flex-col items-center justify-center gap-1 w-full h-full text-text-secondary dark:text-[#baaa9c] hover:text-primary transition-colors">
