...
export function ProductManagementPage() {
    ...
    const filteredProducts = mockProducts.filter(p => {
        if (activeTab === 'habis') return p.isOutOfStock;
        if (activeTab === 'live') return !p.isOutOfStock;
        return true;
    });

    return (
        <MobileContainer className="bg-background-light dark:bg-background-dark min-h-screen flex flex-col pb-24">
            {/* Header Section */}
            <header className="bg-surface-light dark:bg-surface-dark sticky top-0 z-50 shadow-sm transition-colors duration-200">
                ...
                <div className="px-4 py-2 pb-4">
                    <div className="relative flex w-full items-center rounded-lg bg-[#f5f2f0] dark:bg-[#3A2C22] h-10 overflow-hidden group focus-within:ring-2 focus-within:ring-primary/50 transition-all">
                ...
                {/* Tabs */}
                <div className="flex overflow-x-auto hide-scrollbar border-b border-[#e6e0db] dark:border-white/10 px-4 gap-6">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
            ...
            <main className="flex-1 overflow-y-auto">
                ...
                <div className="flex gap-3 p-4 overflow-x-auto hide-scrollbar">
                    <div className="flex min-w-[140px] flex-col justify-between gap-1 rounded-xl p-4 bg-surface-light dark:bg-surface-dark border border-[#e6e0db] dark:border-white/5 shadow-sm">
                        <div className="flex items-center gap-2 text-primary">
                            <Icon name="inventory_2" size={20} />
                            <p className="text-xs font-bold uppercase tracking-wider">Total</p>
                        </div>
                        <p className="text-2xl font-bold leading-tight text-text-main dark:text-white">16</p>
                    ...
                    <div className="flex min-w-[140px] flex-col justify-between gap-1 rounded-xl p-4 bg-surface-light dark:bg-surface-dark border border-[#e6e0db] dark:border-white/5 shadow-sm">
                        ...
                        <p className="text-2xl font-bold leading-tight text-text-main dark:text-white">2</p>
                    ...
                    {filteredProducts.map((product) => (
                        ...
                        >
                            ...
                            <div className="flex items-center justify-end px-3 pb-3 gap-2">
                                {product.isOutOfStock ? (
                                    <>
                                        <button className="flex items-center justify-center px-3 py-1.5 rounded-lg border border-gray-200 dark:border-white/10 text-xs font-bold text-text-main dark:text-white bg-transparent hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                            Hapus
                                        </button>
                                        <button className="flex items-center justify-center px-4 py-1.5 rounded-lg bg-surface-light dark:bg-white/10 border border-primary text-primary hover:bg-primary/5 text-xs font-bold transition-colors gap-1 flex-1">
                                    ...
                                    <>
                                        ...
                                        <button className="flex items-center justify-center px-4 py-1.5 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary text-xs font-bold transition-colors gap-1">
                                            <Icon name="trending_up" size={16} />
                                            Naikkan
                                        </button>
                                        <button className="flex items-center justify-center px-4 py-1.5 rounded-lg bg-primary text-white hover:bg-primary/90 text-xs font-bold transition-colors gap-1 shadow-sm shadow-primary/30">
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-background-light dark:from-background-dark via-background-light dark:via-background-dark to-transparent pt-8 flex justify-center max-w-md mx-auto pointer-events-none">
                <button className="pointer-events-auto flex items-center justify-center w-full h-12 bg-primary text-white rounded-xl shadow-lg shadow-primary/40 active:scale-95 transition-all text-base font-bold gap-2">
